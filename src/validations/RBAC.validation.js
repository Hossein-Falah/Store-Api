...
const updateRoleValidation = Joi.object({
    title: Joi.string().optional().allow("").min(3).max(30).error(createHttpError.BadRequest("عنوان نمی تواند خالی باشد")),
    description: Joi.string().optional().allow("").min(0).max(100).error(createHttpError.BadRequest("توضیحات نمی تواند خالی باشد")),
    permissions: Joi.array().optional().allow("").items(Joi.string().pattern(MongoIDPattern)).error(createHttpError.BadRequest("دسترسی صحیح نمی باشد")),
});
